...
const projects_Dialog = ({ onClose, project, onPrev, onNext }: {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-5 bg-black/75 backdrop-blur-sm">
            <button onClick={onPrev} className="fixed left-24 top-1/2 -translate-y-1/2 w-11 h-11 flex items-center justify-center rounded-full bg-white/5 border border-white/10 text-white hover:bg-indigo-500/50 transition-colors">
                <IconArrowBigLeftLines size={20} />
            </button>
            <button onClick={onNext} className="fixed right-24 top-1/2 -translate-y-1/2 w-11 h-11 flex items-center justify-center rounded-full bg-white/5 border border-white/10 text-white hover:bg-indigo-500/50 transition-colors">
            ...
            <div className="dialog-slide-up relative w-full max-w-2xl bg-[#1a1a1a] border border-[#2a2a2a] rounded-2xl overflow-hidden shadow-2xl">
                <button onClick={onClose} className="absolute top-4 right-4 z-10 w-9 h-9 flex items-center justify-center rounded-full bg-black/50 border border-white/10 text-white hover:bg-indigo-500/50 transition-colors">
                    <IconX size={18} />
                </button>
                ...
                <div className="px-8 py-7 overflow-y-auto max-h-[60vh]">
                    <div className="flex items-center gap-3 mb-3">
                        <h2 className="text-2xl font-bold text-white items-center font-Unbounded">{project.name}</h2>
                        <span className="text-xs font-semibold text-indigo-400 bg-indigo-500/10 border border-indigo-500/20 px-3 py-1 rounded-full font-DM-Sans">{project.category}</span>
                    </div>
                                ...
                                {project.language.map((lang, i) => {
                                    ...
                                    return (
                                        <span key={i} className="flex items-center gap-1.5 text-xs text-white/70 bg-white/5 border border-white/10 px-3 py-1 rounded-md font-DM-Sans">
                                            {Icon && <Icon className={`w-6 h-6 ${lang === 'HTML' ? 'text-orange-400' : ''}`} />}
                                            {lang}
                                        </span>
                                    )
                                })}
